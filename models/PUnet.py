...
from models.UNet_PUnet import Unet
from models.utils import init_weights,init_weights_orthogonal_normal, l2_regularisation
import torch.nn.functional as F
...
class ProbabilisticUnet(nn.Module):
    ...
    def __init__(self, args, input_channels=1, num_classes=4, num_filters=[32,64,128,192], latent_dim=6, no_convs_fcomb=4, beta=10.0):
        ...
        self.Pad = nn.ConstantPad2d((22, 22, 22, 22), 0)
    ...
    def forward(self, patch, segm, training=True):
        ...
        self.unet_features = self.unet.forward(self.Pad(patch),False)[:,:,22:234,22:234]
...
class SetCriterion(nn.Module):
    ...
    def loss_Elbo(self, outputs, targets):
        losses = {
            'loss_Elbo':-outputs['elbo'],
        }
        return losses 
    ...
    def loss_L2reg(self, outputs, targets):
        losses = {
                "loss_L2reg": outputs["reg_loss"],
        }
        return losses
            
    def get_loss(self, loss, outputs, targets):
        loss_map = {'Elbo': self.loss_Elbo,
                    'L2reg': self.loss_L2reg,
...
def build(args):
    ...
    model = ProbabilisticUnet(args, input_channels=1, num_classes=4, num_filters=[32,64,128,192], latent_dim=2, no_convs_fcomb=4, beta=10.0)
    weight_dict = {
        'loss_L2reg': 1e-5,
#        'loss_AvgDice': args.AvgDice_loss_coef,  
        'loss_Elbo':1,
    }
    losses = ['Elbo','L2reg','AvgDice']
